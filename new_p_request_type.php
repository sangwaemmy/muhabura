<?php
    ...
    if (isset($_POST['send_p_request_type'])) {
        if (isset($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'p_request_type') {
                require_once '../web_db/updates.php';
                $upd_obj = new updates();
                $p_request_type_id = $_SESSION['id_upd'];
                $name = $_POST['txt_name'];
                $upd_obj->update_p_request_type($name, $p_request_type_id);
...
<html>
    ...
    <body>
        <form action="new_p_request_type.php" method="post" enctype="multipart/form-data">
            ...
            <div class="parts eighty_centered off saved_dialog">
                p_request_type saved successfully!</div>


            <div class="parts eighty_centered new_data_box off">
                <div class="parts eighty_centered new_data_title">  add more Types of requests Registration </div>
                <table class="new_data_table">
                    ...
                    <tr><td colspan="2"> <input type="submit" class="confirm_buttons" name="send_p_request_type" value="Save"/>  </td></tr>
                </table>
            </div>

            <div class="parts eighty_centered datalist_box" >
                <div class="parts no_shade_noBorder xx_titles no_bg whilte_text dataList_title">All type of requests</div>
                <?php
                    $obj = new multi_values();
                    $first = $obj->get_first_p_request_type();
                    $obj->list_p_request_type($first);
                ?>
            </div>  
    ...
    <table class="margin_free">
        <td>
            <form action="../web_exports/excel_export.php" method="post">
                <input type="hidden" name="p_request_type" value="a"/>
                <input type="submit" name="export" class="btn_export btn_export_excel" value="Export"/>
            </form>
        </td>
        <td>
            <form action="../prints/print_p_request_type.php" method="post">
    ...
    function chosen_name_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'p_request_type') {
                ...
                return $name->get_chosen_p_request_type_name($id);
            } else {
                return '';
            }
        } else {
