<?php
    ...
    if (isset($_POST['send_sales_quote_header'])) {
        if (isset($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                require_once '../web_db/updates.php';
                $upd_obj = new updates();
                $sales_quote_header_id = $_SESSION['id_upd'];
                ...
                $upd_obj->update_sales_quote_header($customer, $date, $payment_term, $reference_number, $number, $status, $sales_quote_header_id);
            ...
            $obj = new new_values();
            $obj->new_sales_quote_header($customer, $date, $payment_term, $reference_number, $number, $status);
        }
    }
?>
...
<html>
    ...
    <body>
        ...
        <!--End Tuype Details-->   
        <form action="new_sales_quote_header.php" method="post" enctype="multipart/form-data">
            ...
            <div class="parts eighty_centered off saved_dialog">
                sales_quote_header saved successfully!</div>


            <div class="parts eighty_centered new_data_box off">
                <div class="parts eighty_centered new_data_title">  sales_quote_header Registration </div>
                <table class="new_data_table">




                    <tr><td colspan="2"> <input type="submit" class="confirm_buttons" name="send_sales_quote_header" value="Save"/>  </td></tr>
                </table>
            </div>

            <div class="parts eighty_centered datalist_box" >
                <div class="parts no_shade_noBorder xx_titles no_bg whilte_text dataList_title">sales_quote_header List</div>
                <?php
                    $obj = new multi_values();
                    $first = $obj->get_first_sales_quote_header();
                    $obj->list_sales_quote_header($first);
                ?>
            </div>  
        </form>
            ...
            <table class="margin_free">
                <td>
                    <form action="../web_exports/excel_export.php" method="post">
                        <input type="hidden" name="sales_quote_header" value="a"/>
                        <input type="submit" name="export" class="btn_export btn_export_excel" value="Export"/>
                    </form>
                </td>
                <td>
                    <form action="../prints/print_sales_quote_header.php" method="post">
    ...
    function chosen_customer_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $customer->get_chosen_sales_quote_header_customer($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_date_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $date->get_chosen_sales_quote_header_date($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_payment_term_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $payment_term->get_chosen_sales_quote_header_payment_term($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_reference_number_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $reference_number->get_chosen_sales_quote_header_reference_number($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_number_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $number->get_chosen_sales_quote_header_number($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_status_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'sales_quote_header') {
                ...
                return $status->get_chosen_sales_quote_header_status($id);
            } else {
                return '';
            }
        } else {
