<?php
    ...
    if (isset($_POST['send_p_district'])) {
        if (isset($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'p_district') {
                require_once '../web_db/updates.php';
                $upd_obj = new updates();
                $p_district_id = $_SESSION['id_upd'];
                $name = $_POST['txt_name'];
                $province = $_POST['txt_province_id'];
                $upd_obj->update_p_district($name, $province, $p_district_id);
                unset($_SESSION['table_to_update']);
            }
        } else {
            $name = $_POST['txt_name'];
            $province = trim($_POST['txt_province_id']);

            require_once '../web_db/new_values.php';
            $obj = new new_values();
            $obj->new_p_district($name, $province);
        }
    }
?>
...
<html>
    ...
    <body>
        ...
        <!--End Tuype Details-->
        <form action="new_p_district.php" method="post" enctype="multipart/form-data">
            <input type="hidden" id="txt_shall_expand_toUpdate" value="<?php echo (isset($_SESSION['table_to_update'])) ? trim($_SESSION['table_to_update']) : '' ?>" />
            <!--this field  (shall_expand_toUpdate)above is for letting the form expand using js for updating-->

            <input type="hidden" id="txt_province_id"   name="txt_province_id"/>
            <?php
            ...
            <!--End dialog--><div class="parts eighty_centered no_paddin_shade_no_Border hider_box">  
                <div class="parts  no_paddin_shade_no_Border new_data_hider"> Add More Locations </div>  </div>
            <div class="parts eighty_centered off saved_dialog">
                p_district saved successfully!</div>
            <div class="parts eighty_centered new_data_box off">
                <div class="parts eighty_centered new_data_title">  Add more districts </div>
                <table class="new_data_table">
                    ...
                    <tr><td class="new_data_tb_frst_cols">Province </td><td> <?php get_province_combo(); ?>  </td></tr>

                    <tr><td colspan="2"> <input type="submit" class="confirm_buttons" name="send_p_district" value="Save"/>  </td></tr>
                </table>
            </div>

            <div class="parts eighty_centered datalist_box" >
                <div class="parts no_shade_noBorder xx_titles no_bg whilte_text dataList_title">All districts</div>
                <?php
                    $obj = new multi_values();
                    $first = $obj->get_first_p_district();
                    $obj->list_p_district($first);
                ?>
            </div>  
        </form>
            ...
            <table class="margin_free">
                <td>
                    <form action="../web_exports/excel_export.php" method="post">
                        <input type="hidden" name="p_district" value="a"/>
                        <input type="submit" name="export" class="btn_export btn_export_excel" value="Export"/>
                    </form>
                </td>
                <td>
                    <form action="../prints/print_p_district.php" method="post">
    ...
    function chosen_name_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'p_district') {
                ...
                return $name->get_chosen_p_district_name($id);
            } else {
                return '';
            }
        } else {
    ...
    function chosen_province_upd() {
        if (!empty($_SESSION['table_to_update'])) {
            if ($_SESSION['table_to_update'] == 'p_district') {
                ...
                return $province->get_chosen_p_district_province($id);
            } else {
                return '';
            }
        } else {
